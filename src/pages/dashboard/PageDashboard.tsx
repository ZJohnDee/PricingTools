...
const PageDashboard: FC = (props: any) => {
  ...
  const reload = () => {
    ...
    if (tempUser === null) {
      console.log("User is null. Trying to refetch the user");
      tempUser = auth.currentUser;

      if (tempUser === null)
      {
        console.log("Refetched user is also null.");
        return;
      }
    }
    ...
    getAllProductsFromFirestore(tempUser).then((result) => {
      ...
      for (let i = 0; i < result.length; i++) {
        ...
        setEmProducts(emProducts.map((val) => {
          return val
        }));
      }
      ...
      getAllContractsFromFirestore(tempUser).then((result) => {
        if (result != null) {
          ...
          for (let i = 0; i < result.length; i++) {
            ...
            setEmContracts(emContracts.map((val) => {
              return val
            }));
          }
        }
        ...
        getAllClientsFromFirestore(tempUser).then((result) => {
          ...
          for (let i = 0; i < result.length; i++) {
            ...
            setEmClients(emClients.map((val) => {
              return val
            }));
          }
    ...
    <div className={"page-dashboard"}>
      ...
      <div className={"page-dashboard-sections"}>
        <div className={"page-dashboard-section edit-component-sub page-dashboard-products"}>
          ...
          {emProducts}
          <Button style={buttonStyle} color={"secondary"} variant={"contained"}
                  href={"/add/product"}>{langProvider.getText("Buttons.Add")}</Button>
        </div>
        <div className={"page-dashboard-section edit-component-sub page-dashboard-contracts"}>
          ...
          {emContracts}
          <Button style={buttonStyle} color={"secondary"} variant={"contained"}
                  href={"/add/contract"}>{langProvider.getText("Buttons.Add")}</Button>
        </div>
        <div className={"page-dashboard-section edit-component-sub page-dashboard-customers"}>
          ...
          {emClients}
          <Button style={buttonStyle} color={"secondary"} variant={"contained"}
                  href={"/add/client"}>{langProvider.getText("Buttons.Add")}</Button>
        </div>
      </div>
